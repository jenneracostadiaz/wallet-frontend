<section>
    ...
    <x-categories-table :categories="$categories" :count="$count" :total_category="$total_category" />


    <x-modal :maxWidth="'lg'" wire:model="modal">
        <x-modal-content :name="$type === 'expense' ? __('Categories Expenses') : __('Categories Income')">
            <div class="flex items-center justify-center mb-4">
                <button type="button" wire:click="previousMonth" class="px-4 py-2 text-xs border-b border-slate-700"> {{ __('← prev') }}</button>
                <span class="mx-4 text-sm font-semibold">
                    {{ \Carbon\Carbon::create($filterYear, $filterMonth)->format('F Y') }}
            ...
            <x-categories-table :categories="$allCategories" :count="$count" :total_category="$total_category" />
        </x-modal-content>
    </x-modal>
